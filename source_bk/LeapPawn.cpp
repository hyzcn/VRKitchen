...
void ALeapPawn::UpdateAnim()
{
	...
	{
		FireGrabEvents(LeftHand, LeftGrabPress);
	}
	if (RightHand->Confidence > 0)
	{
		FireGrabEvents(RightHand, RightGrabPress);
	}
...
// Fire grab event based on touch controller input
void ALeapPawn::FireGrabEvents(UHandObject * Hand, bool pressed)
{
	if (pressed)
	{
		Grab(Hand->IsLeft);
...
bool ALeapPawn::GetClosestHand(AMovableItem* GrabObject)
{
	float LeftDist, RightDist;
	LeftDist = FVector::Dist(LeftHand->HandLocation, GrabObject->GetActorLocation());
	RightDist = FVector::Dist(RightHand->HandLocation, GrabObject->GetActorLocation());
...
void ALeapPawn::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	Super::SetupPlayerInputComponent(InputComponent);
	InputComponent->BindAction("GrabLeft", EInputEvent::IE_Pressed, this, &ALeapPawn::GrabLeft);
	InputComponent->BindAction("GrabLeft", EInputEvent::IE_Released, this, &ALeapPawn::ReleaseLeft);
	InputComponent->BindAction("GrabRight", EInputEvent::IE_Pressed, this, &ALeapPawn::GrabRight);
	InputComponent->BindAction("GrabRight", EInputEvent::IE_Released, this, &ALeapPawn::ReleaseRight);
}

void ALeapPawn::GrabLeft()
{
	LeftGrabPress = true;
}

void ALeapPawn::GrabRight()
{
	RightGrabPress = true;
}

void ALeapPawn::ReleaseLeft()
{
	LeftGrabPress = false;
}

void ALeapPawn::ReleaseRight()
{
	RightGrabPress = false;
}