...
def run_episodes(agent):
    ...
    agent.load('data/%s-%s-model-%s.bin' % (agent_type, config.tag, agent.task.name))
    while True:
        ep += 1
        episode_start_time = time.time()
        reward, step, loss, t_steps = agent.episode()
        episode_time = time.time()-episode_start_time
        config.logger.scalar_summary("run time", episode_time, ep)
        config.logger.scalar_summary("loss", loss, ep)
        config.logger.scalar_summary("step time", t_steps, ep)
